 ...
 *
 * 新契约（本文件 RED 部分）：
 * - subagent 工具参数 schema 声明 action 字段（dispatch/status/cancel，缺省 dispatch）
 * - subagent_status / subagent_cancel 不再单独注册
 * - 工具 description 不再引用已移除的独立工具名（subagent_status/subagent_cancel），
 *   但保留不轮询指引与取消防滥用引导
 * - action=status：返回在途列表（不要求 agent/task，不触发深度限制）
...
describe("单入口 action 模式 — 新契约红阶段", () => {
	...
	describe("1. 单入口 action 参数（新契约）", () => {
		it("schema：agent/task 应为可选，action 枚举含 dispatch/status/cancel 且默认 dispatch（RED：agent/task 仍必填）", () => {
			...
			// action：可选（不在 required 中）、枚举含 dispatch/status/cancel、默认 dispatch
			...
			expect(actionConsts).toEqual(expect.arrayContaining(["dispatch", "status", "cancel"]));
			expect(props.action.default).toBe("dispatch");

			// agent/task：修复方向为 Optional（status/cancel 不要求 agent/task）
			// → 不应出现在 required 中（RED：当前仍为必填）
	...
	// ================================================================
	// 3. 新契约：action=status 返回在途列表
	// ================================================================
	describe("3. action=status 返回在途列表（新契约）", () => {
		it("action=status 应返回在途任务的 taskId、agent、任务描述（RED：当前报缺 agent）", async () => {
			const { executeSubagentTool } = setupExtension();
			const ctx = createMockTuiCtx(defaultCwd);
			...
			const p1 = executeSubagentTool!(
				"call-1",
				{ agent: "tester", task: "调研 XX 方案", sessionId: "action-status-1" },
				undefined,
				undefined,
				ctx,
			);
			...
			const p2 = executeSubagentTool!(
				"call-2",
				{ agent: "tester", task: "重构认证中间件", sessionId: "action-status-2" },
				undefined,
				undefined,
				ctx,
			);
			...
			// Act: action=status，不携带 agent/task
			const result = await executeSubagentTool!("call-3", { action: "status" }, undefined, undefined, ctx);

			// Assert: 不应是参数缺失错误，而应返回在途列表
			expect(result.isError).toBeFalsy();
			const text = result.content.map((c: any) => c.text).join("");
			expect(text).toContain("action-status-1");
			expect(text).toContain("action-status-2");
			expect(text).toContain("tester");
			expect(text).toMatch(/调研 XX 方案/);
			expect(text).toMatch(/重构认证中间件/);
		});

		it("action=status 无在途任务时应返回「当前无在途任务」（RED）", async () => {
			const { executeSubagentTool } = setupExtension();
			const ctx = createMockTuiCtx(defaultCwd);
		...
		it("action=status 应只列 status=running 的任务（RED）", async () => {
			const { executeSubagentTool } = setupExtension();
			const ctx = createMockTuiCtx(defaultCwd);

			// Arrange: dispatch 2 tasks, cancel one
			const p1 = executeSubagentTool!(
				"call-1",
				{ agent: "tester", task: "任务1", sessionId: "action-status-running" },
				undefined,
				undefined,
				ctx,
			);
			...
			const p2 = executeSubagentTool!(
				"call-2",
				{ agent: "tester", task: "任务2", sessionId: "action-status-cancelled" },
				undefined,
				undefined,
				ctx,
			);
			...
			// Cancel via 新契约 action=cancel
			await executeSubagentTool!(
				"call-3",
				{ action: "cancel", taskId: "action-status-cancelled" },
				undefined,
				undefined,
				ctx,
			);
			...
			const text = result.content.map((c: any) => c.text).join("");
			expect(text).toContain("action-status-running");
			expect(text).not.toContain("action-status-cancelled");
		});

		it("action=status 返回 details.activeTasks 结构化数组（RED）", async () => {
			const { executeSubagentTool } = setupExtension();
			const ctx = createMockTuiCtx(defaultCwd);

			const p1 = executeSubagentTool!(
				"call-1",
				{ agent: "tester", task: "任务", sessionId: "action-status-details" },
				undefined,
				undefined,
				ctx,
			);
			...
			expect(result.details?.activeTasks).toEqual(["action-status-details"]);
		});

		it("action=status 不应包含耗时格式（RED）", async () => {
			const { executeSubagentTool } = setupExtension();
			const ctx = createMockTuiCtx(defaultCwd);

			const p1 = executeSubagentTool!(
				"call-1",
				{ agent: "tester", task: "测试任务", sessionId: "action-status-no-elapsed" },
				undefined,
				undefined,
				ctx,
			);
		...
		it("深度限制应拦截 action=status（depth=1 子 agent 查询应收到深度拦截错误，RED：当前 status 绕过深度检查）", async () => {
			...
			// 子 agent 不应拿到在途列表/「当前无在途任务」，而应收到深度拦截错误
			expect(result.isError).toBe(true);
			const text = result.content.map((c: any) => c.text).join("");
